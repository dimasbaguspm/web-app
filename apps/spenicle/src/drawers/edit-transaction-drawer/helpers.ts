...
export const formatDefaultValues = (
  ...
  if (isPayloadExist) {
    return {
      ...
      destinationAccountId: payload?.destinationAccountId ? +payload.destinationAccountId : 0,
      categoryId: payload?.categoryId ? +payload.categoryId : 0,
      amount: payload?.amount ? (isNaN(+payload?.amount) ? 0 : +payload.amount!) : 0,
      notes: typeof payload?.notes === 'string' ? payload?.notes : '',
    };
  } else {
    return {
      type: transaction?.type ?? 'expense',
      date: formatDate(transaction?.date ?? dayjs().toISOString(), DateFormat.ISO_DATE),
      time: formatDate(transaction?.date ?? dayjs().toISOString(), DateFormat.TIME_24H),
